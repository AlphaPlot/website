...
_5gridC.prototype.trigger = function (name) {
    if (!this.isReady || !this.events[name] || this.events[name].length < 1) return;
...
(function () {
    ...
    jQuery.fn.tap_5grid = function (f, suffix, noMove) {
        ...
        if (isTouch) {
            target.bind('touchstart' + (suffix ? '.' + suffix : ''), function () {
                if (!isTapLocked) {
                    isTapLocked = true;
                    if (_settings.mobileUI_tapDelay > -1) tapId = window.setTimeout(function () {
            ...
            if (noMove) target.bind('touchmove' + (suffix ? '.' + suffix : ''), function (e) {
                isTapLocked = false;
                window.clearTimeout(tapId);
            });
        }
    ...
    jQuery.fn.untap_5grid = function (suffix) {
        var target = jQuery(this);
        if (isTouch) {
            target.unbind('touchstart' + (suffix ? '.' + suffix : '')).unbind('touchend' + (suffix ? '.' + suffix : '')).unbind('touchmove' + (suffix ? '.' + suffix : ''));
        }
    ...
    headQueue.process_5grid = function () {
        var doE;
        if (document.createStyleSheet) doE = function (s) {
        ...
        for (x in headQueue) {
            if (headQueue[x].type == 'i') _head.append('<style>' + headQueue[x].value + '</style>');
    ...
    if (doMobile && !doFluid && !do1000px && !do1200px && !doDesktop) doMobileOnly = true;
    // Apply workarounds for broken/old browsers
    // Android, Webkit <= 534
    if (navigator.userAgent.match(/Android.+AppleWebKit\/534/)) _settings.mobileUI_titleBarFixed = 0;
    // IE <= 8
    if (jQuery.browser.msie) {
        // HTML5 Shiv
        if (jQuery.browser.version < 9) _head.append('<script type="text/javascript" src="' + baseURL + '5grid/html5shiv.js" />');
    ...
    (function () {
        ...
        if (screen.width > screen.height && Math.abs(orientation) == 90) sw = screen.height;
        // Mobile (exclusive)
        if ((doMobile && (ww <= _settings.breakpoint_mobile || sw <= _settings.breakpoint_mobile)) || doMobileOnly) {
            ...
            if (_settings.mobileUI == 1) {
                ...
                if (_settings.mobileUI_theme != 'none') {
                    ...
                    if (_settings.mobileUI_themeTitleBarColor) headQueue.pushI_5grid('#mobileUI-site-titlebar { background-color: ' + _settings.mobileUI_themeTitleBarColor + '; }');
                    if (_settings.mobileUI_themeNavColor) headQueue.pushI_5grid('#mobileUI-site-nav { background-color: ' + _settings.mobileUI_themeNavColor + '; }');
                }
            }
            ...
            // Desktop
                ...
                if (ww >= _settings.breakpoint_1000px) {
                    ...
                    if (do1200px) headQueue.pushE_5grid(baseURL + _settings.prefix + '-1200px.css');
                }
                // 1000px
                else {
                    ...
                    if (do1000px) headQueue.pushE_5grid(baseURL + _settings.prefix + '-1000px.css');
                }
            }
            else {
                // 1000px (exclusive)
    ...
    /* MobileUI          */
    /*********************/
    if (_settings.mobileUI_force == 1) jQuery(function () {
        var _body = jQuery('body')
            , _viewer
            , _wrapper
            , _nav
        ...
        if (_main_content.length > 0)
            if (_main_content_target.length > 0) _main_content.prependTo(_main_content_target);
        ...
        jQuery('.mobileUI-site-nav a').each(function () {
            ...
            site_nav_options.push('<div class="mobileUI-site-nav-link" xhref="' + t.attr('href') + '"><span class="indent-' + indent + '"></span>' + t.text() + '</div>');
        });
        // Configure elements
        ...
        // Wrapper
        _wrapper.accelerate_5grid().css('position', 'absolute').width(_wrapper_width);
        // Nav
        _nav.accelerate_5grid().width(_nav_width).prependTo(_wrapper).css('position', 'absolute').css('left', (-1 * _nav_width)).prepend('<div style="position: absolute; top: 0px; right: -1px; width: 1px; height: 1px;"></div>');
        // Inner
        _nav_inner.css('overflow', (isTouch ? 'hidden' : 'auto')).bind('touchstart', function (e) {
        ...
        // Links
        _nav.find('.mobileUI-site-nav-link').disableSelection_5grid().css('cursor', 'pointer').each(function () {
            var t = jQuery(this);
            t.tap_5grid(function (e) {
                ...
                if (_nav_isOpen) {
                    ...
                    if (href && href.length > 0) _body.trigger('5grid_closeNav', [href]);
                }
            });
        });
        // Opener
        _opener.accelerate_5grid().css('position', 'absolute').css('z-index', 10001).css('cursor', 'pointer').disableSelection_5grid().appendTo(_titlebar).tap_5grid(function (e) {
            e.stopPropagation();
            e.preventDefault();
            _body.trigger('5grid_toggleNav');
        }, '', true);
        // Titlebar
        _titlebar.accelerate_5grid().width(_content_width).height(_settings.mobileUI_titleBarHeight).css('z-index', 10000).prependTo(_viewer);
        if (_settings.mobileUI_titleBarFixed) {
            _titlebar.bind('goActive_5grid', function () {
                _titlebar.css('position', 'fixed').css('top', 0).css('left', 0);
            }).bind('goInactive_5grid', function () {
                _titlebar.css('position', 'absolute').css('top', _window.scrollTop()).css('left', _nav.width);
        ...
        if (_settings.mobileUI_titleBarOverlaid) _content.bind('resize', function () {
            _viewer.height(_content.height());
        });
        else _content.css('padding-top', _settings.mobileUI_titleBarHeight).bind('resize', function () {
            _viewer.height(_content.height() + _settings.mobileUI_titleBarHeight);
        });
        // Viewer
        _viewer.css('position', 'absolute').css('overflow', 'hidden').width(_content_width).height(_content.height() + _settings.mobileUI_titleBarHeight);
        // Body
        _body.bind('5grid_toggleNav', function () {
            ...
            _nav_inner.height(window.innerHeight - _settings.mobileUI_navInnerPad).scrollTop(0);
            // Deactivate titlebar
            _titlebar.trigger('goInactive_5grid');
            // Animate
            _wrapper.add(_titlebar).animate({
                left: _nav_width
            ...
            _wrapper.add(_titlebar).animate({
                ...
                if (url) window.setTimeout(function () {
        ...
        // Remove address bar
        if (_settings.mobileUI_hideAddressBar == 1) _window.load(function () {
    ...
    _5grid.isReady = true;
    jQuery(function () {
        if (_5grid.readyWait) jQuery(window).load(function () {
            _5grid.readyCheck();
        });
